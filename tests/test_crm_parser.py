...
class TestClawback:
    ...
    def test_clawback_applied_into_the_same_period_as_genuine_activity(self):
        """If the client's own Dropped Date month happens to coincide with
        the SAME agent's genuinely-cleared month in this same file, the
        clawback and the fresh commission land in the same period entry —
        no separate holding entry needed."""
        data = crm_csv([
            client("N1", cleared="08/05/2026", debt="20000"),   # new this month
            client("A2", cleared="06/12/2026", dropped="08/03/2026",
                   payments="1", debt="10000"),                 # dropped this month
        ])
        periods = by_period(parse_crm_and_calculate(
            data, "f.csv", already_cleared_crm_ids={"A2"}))

        assert set(periods) == {"2026-08"}
        august = periods["2026-08"]["results"][0]
        assert august["units_cleared"] == 1                        # N1 only
        assert august["gross_commission"] == pytest.approx(200.0)  # 20,000 x 1%
        assert august["clawback_amount"] == pytest.approx(100.0)   # fallback: 10,000 x 1%
        assert august["net_commission"] == pytest.approx(100.0)
