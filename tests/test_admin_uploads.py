"""Tests for the index page's "recent uploads" lists and their delete/reset
actions — lets a wrong upload (CRM file, Cordoba payout file, or commission-
history ledger) be fully undone without shelling into the DB. CRM export and
history-backfill deletes just reuse the existing well-tested period-cascade
delete, grouped by filename. The Cordoba delete is the sensitive one: it has
to reverse real clawback dollars, not just remove ledger rows, so it gets the
most coverage here."""
...
class TestDeleteCordobaUpload:
    def test_reverses_a_clawback_and_removes_the_holding_period(self, db):
        ...
        result = _delete_cordoba_upload(FAKE_FILE.filename)
    ...
    def test_does_not_touch_a_real_period_with_other_agents(self, db):
        ...
        _delete_cordoba_upload(FAKE_FILE.filename)
    ...
    def test_unrelated_files_untouched(self, db):
        ...
        _delete_cordoba_upload("file_a.xlsx")
...
class TestGroupAndDeletePeriodsByFilename:
    ...
    def test_group_periods_by_filename_groups_multi_month_upload(self, db):
        ...
        groups = _group_periods_by_filename(CommissionPeriod.query.all())
    ...
    def test_delete_periods_by_filename_only_deletes_matching_source(self, db):
        self._make_period(db, "2026-01", "crm_export.csv", source="crm")
        self._make_period(db, "2025-06", "crm_export.csv", source="history_import")

        deleted = _delete_periods_by_filename("crm_export.csv", ("crm", "manual"))
...
class TestUploadRoutes:
    def test_delete_crm_upload_route(self, app, db, client):
        ...
        resp = client.post(
            "/uploads/crm/delete", data={"filename": "crm_export.csv"}, follow_redirects=True,
        )
    ...
    def test_delete_cordoba_upload_route(self, app, db, client):
        ...
        resp = client.post(
            "/uploads/cordoba/delete", data={"filename": FAKE_FILE.filename}, follow_redirects=True,
        )
