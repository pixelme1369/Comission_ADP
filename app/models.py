...
class ClientRecord(db.Model):
    ...
    # Cordoba (funder) payout reconciliation: has Cordoba ever confirmed paying us on this file?
    cordoba_paid = db.Column(db.Boolean, default=False)


class CordobaPaidClient(db.Model):
    """Ledger of every client ID Cordoba has ever confirmed funding (First Pays or EPF tab)."""
...
class CordobaChargeback(db.Model):
    """
    One row per client Cordoba charged back from us (Chargebacks tab of the weekly payout file).
    Kept separate from the app's own CRM-predicted ClientRecord clawback so the two can be
    compared side by side instead of merged.
    """
    __tablename__ = "cordoba_chargeback"
    ...
    dropped_date = db.Column(db.String(50))
    clawback_based_on_payout_amount = db.Column(db.Float, default=0.0)
    matched = db.Column(db.Boolean, default=False)  # False if crm_id had no matching ClientRecord
    uploaded_filename = db.Column(db.String(255))
    uploaded_at = db.Column(db.DateTime, default=lambda: datetime.now(timezone.utc))
