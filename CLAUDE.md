...
**Clawback rate (OWNER POLICY, revised — supersedes the original "tier recalculation" rule that used to live here):** the clawback is always just the client's own proportional share of whatever rate the original month was actually paid at — `enrolled_debt × orig_rate`, where `orig_rate = orig_gross_commission / orig_total_debt`. This applies whether that original month's math came from the standard tier table, a fixed-rate agent, or a blended rate summed across multiple sources sharing a period_label (`known_period_totals`) — they all collapse to the same gross/debt ratio. The old rule ("if removing this client would have dropped the whole month below a tier boundary, claw back the full commission difference across ALL of that month's debt, not just this client's share") is **removed** — it could produce a clawback wildly out of proportion to the dropped client's own debt: a real case surfaced an $18,007 client generating a $5,364.47 clawback (~30% effective rate, nowhere near any real tier rate) because their departure alone, in isolation, would have dropped the agent below a tier boundary for a month's worth of *other* clients' debt they had nothing to do with. If the agent has no commission result at all for the original cleared month (e.g. `orig_total_debt` is 0 — they had 0 net cleared units there after other cancels, or the only other activity was a $0-commission low-credit/safe-cancel unit), the clawback falls back to a flat `enrolled_debt × 1%` (lowest tier rate). Regression-tested in `tests/test_calculator.py::TestClawbackAmount::test_tier_change_no_longer_reprices_the_whole_month` (both apps).

Fixed-rate agents (Alex Tambouly, Peter Godwin) are unaffected either way — checked first, unconditional flat rate, no tier to recalculate before or after this policy change (see "Per-agent fixed-rate override" below).

A client whose original commission came from a Commission History import with the owner-added "Rate" column carries an even more precise number than `orig_rate` above can reconstruct: `ClientRecord.paid_rate`, the exact rate that row was actually paid at. When known, `crm_parser.py` Step 3 uses `enrolled_debt × paid_rate` verbatim, bypassing `calculate_clawback_amount` entirely for that client. See `known_rate_by_crm_id` in `commission_core/crm_parser.py`.
