...
from agent_portal.cordoba_ingest import _apply_cordoba_chargebacks, _list_cordoba_chargebacks
...
def test_never_paid_client_is_not_listed_as_cordoba_chargeback(db):
    ...
    db.session.add(ClientRecord(
        ...
        pay_freq="Semi-Monthly", payments_made=1,
    ))
    db.session.commit()

    listed, updated, total, skipped = _list_cordoba_chargebacks(
        FAKE_FILE, parsed([chargeback_row(name="Patrick Keiran")]))

    assert (listed, updated, total) == (0, 0, 0.0)
    assert skipped == [f"Patrick Keiran (ID {CRM_ID})"]
